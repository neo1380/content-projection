...
if (!isSSR()) {
  PDFJS = require('./pdf-dist/build/pdf');
  PDFJSViewer = require('./pdf-dist/web/pdf_viewer');
...
@Component({
  selector: 'ipdf-viewer',
  template: `
    <div #ipdfViewerContainer class="ng2-ipdf-viewer-container">
      <div class="pdfViewer"></div>
    </div>
  `,
  styleUrls: ['./ipdfviewer.component.scss']
})
export class IPdfViewerComponent
  implements OnChanges, OnInit, OnDestroy, AfterViewChecked {
  @ViewChild('ipdfViewerContainer', { static: false }) ipdfViewerContainer;
  ...
  static setExternalLinkTarget(type: string) {
    const linkTarget = IPdfViewerComponent.getLinkTarget(type);
  ...
  ngAfterViewChecked(): void {
    ...
    const offset = this.ipdfViewerContainer.nativeElement.offsetParent;
    ...
    this._pdf
      ...
      .then((page: PDFPageProxy) => {
        const viewportWidth =
          ...
          }).width * IPdfViewerComponent.CSS_UNITS;
        ...
        if (
          !this._originalSize ||
          (this._fitToPage &&
            viewportWidth > this.ipdfViewerContainer.nativeElement.clientWidth)
        ) {
          scale = this.getScale(
  ...
  private setupMultiPageViewer() {
    ...
    IPdfViewerComponent.setExternalLinkTarget(this._externalLinkTarget);
  ...
  private getScale(viewportWidth: number) {
    const pdfContainerWidth =
      this.ipdfViewerContainer.nativeElement.clientWidth -
      (this._showBorders ? 2 * IPdfViewerComponent.BORDER_WIDTH : 0);
